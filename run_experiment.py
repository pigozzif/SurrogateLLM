...
def construct_tsp(params):
    ...
    if '--tsplib-file' in params:
        return [load_tsplib(params['--tsplib-file'], iters, noise_seed) for (iters, noise_seed) in
                product(iter_ns, noise_seeds)]
    elif '--explicit-file' in params:
        return [load_explicit_tsp(params['--explicit-file'], iters, noise_seed) for (iters, noise_seed) in
                product(iter_ns, noise_seeds)]
    else:
...
def construct_linearmivabo(params):
    ...
    return [Linear(n_vars=num_vars_d + num_vars_c, n_vars_d=num_vars_d, noisy=noisy, laplace=laplace, seed=seed) for
            seed in seeds]
...
def execute_IDONE(params, problem, max_eval, log):
    from expensiveoptimbenchmark.solvers.IDONE.wIDONE import optimize_IDONE
    ...
    return optimize_IDONE(problem, max_eval, rand_evals=rand_evals, model=type_model,
                          binarize_categorical=binarize_categorical, binarize_int=binarize_int, sampling=sampling,
                          enable_scaling=enable_scaling, log=log, exploration_prob=expl_prob, idone_log=idone_log)


## MVRSM
def execute_MVRSM(params, problem, max_eval, log):
    from expensiveoptimbenchmark.solvers.MVRSM.wMVRSM import optimize_MVRSM
    if params['--model'] not in ['basic', 'advanced']:
        raise ValueError("Valid model types are `basic` and `advanced`")
    if params['--binarize-categorical'] not in ['true', 't', 'yes', 'y', 'false', 'f', 'no', 'n']:
        raise ValueError("--binarize-categorical should be a boolean.")
    ...
    return optimize_MVRSM(problem, max_eval, rand_evals=rand_evals, model=type_model,
                          binarize_categorical=binarize_categorical, enable_scaling=enable_scaling, optimizer=optimizer,
                          bound_h=bound_h, log=log)


## SA
def execute_SA(params, problem, max_eval, log):
    from expensiveoptimbenchmark.solvers.SA.wSA import optimize_SA
...
## DONE
def check_DONEjl():
    from expensiveoptimbenchmark.solvers.DONEjl.wDONEjl import minimize_DONEjl
    import numpy as np
...
def execute_DONEjl(params, problem, max_eval, log):
    from expensiveoptimbenchmark.solvers.DONEjl.wDONEjl import optimize_DONEjl
...
def execute_hyperopt(params, problem, max_eval, log):
    from expensiveoptimbenchmark.solvers.hyperopt.whyperopt import optimize_hyperopt_tpe
    rand_evals = int(params['--rand-evals'])
...
def execute_pygpgo(params, problem, max_eval, log):
    from expensiveoptimbenchmark.solvers.pyGPGO.wpyGPGO import optimize_pyGPGO
...
def execute_smac(params, problem, max_eval, log):
    from expensiveoptimbenchmark.solvers.smac.wsmac import optimize_smac
    rand_evals = int(params['--rand-evals'])
...
def execute_cocabo(params, problem, max_eval, log):
    from expensiveoptimbenchmark.solvers.CoCaBO.wCoCaBo import optimize_CoCaBO
    rand_evals = int(params['--rand-evals'])
...
def execute_cma(params, problem, max_eval, log):
    from expensiveoptimbenchmark.solvers.CMA.wCMA import optimize_CMA
    bound_h = params['--boundary-handler']
...
def execute_scipy_basinhopping(params, problem, max_eval, log):
    from expensiveoptimbenchmark.solvers.scipy.wscipy import optimize_basinhopping
    T = float(params['--T'])
...
def execute_scipy_local(params, problem, max_eval, log):
    from expensiveoptimbenchmark.solvers.scipy.wscipy import optimize_scipy_local
    method = params['--method']
...
## Vanilla GA
def execute_ga(params, problem, max_eval, log):
    from wga import optimize_GA
    return optimize_GA(problem=problem,
                       max_evals=max_eval,
...
if len(args) == 1 or (len(args) == 2 and (args[1] == '-h' or args[1] == '--help')):
    ...
    print(
        f" --explicit-file=<path> \t Parse and load an explicit TSP instance with filename as name and an explicit W as content")
    ...
    print(
        f" --sampling=<none|thompson|uniform> \t Whether to use thompson or uniform sampling, or none. (default: none)")
    ...
    print(
        f" --expl-prob=<normal|larger> \t Decide probability of exploration. Not applicable to Thompson sampling. (default: normal)")
    ...
    print(f" hyperopt")
    print(
        f" --int-conversion-mode=<quniform|randint> \t The default conversion mode for integers for hyperopt (default: quniform)")
