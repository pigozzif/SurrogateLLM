...
def run_problems(args):
    ...
    if pidx == 0:
        ...
        os.system("python run_experiment.py --max-eval=1000 --seed={0} pitzdaily {1}".format(config.s, config.solver))
    elif pidx == 3:
        os.system("python run_experiment.py --max-eval=1000 --seed={0} --folder=expensiveoptimbenchmark/problems/steel+plates+faults hpo {1}".format(config.s, config.solver))
